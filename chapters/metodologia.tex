...
As pesquisas podem possuir diversas categorias na metodologia científica, algumas delas são: quanto a finalidade, quanto a objetivos, quanto a procedimentos e quanto a natureza da abordagem. Desse modo, podemos definir exemplos dessas categorias relevantes para este texto. Logo, quanto a finalidade há a pesquisa aplicada que possui foco na aplicação, utilização e consequências práticas do conhecimento \cite{gil2008metodos}. Quanto aos objetivos destaca-se a pesquisa descritiva que tem como objetivo primordial a descrição das características de determinada população ou fenômeno \cite{gil2002como}. Quanto aos procedimentos: a pesquisa experimental, determina um objeto de estudo e váriaveis que são capazes de influenciá-lo e define formas de controle e observação dos efeitos que a manipulação dessas variáveis possuem no objeto. E, quanto a natureza da abordagem é de destaque a qualitativa onde a análise é sistemática e compreensiva, usada usualmente em estudos de caso e pesquisa-ação, pois não há fórmula para orientar o pesquisador, assim também precisa ser maleável \cite{gil2008metodos}.
...
Essas métricas vão ser acessadas através de testes realizados em uma rede, simulando um contexto \en{Peer-to-Peer}. Assim, para o ambiente de simulação de \en{nodes} cabe observar os seguintes casos para a coleta de dados: atualização de dados, resolução de conflitos, publicar ou convergir estados (em busca de consistência); Tais observações testam a capacidade do sistema de manter a integridade do fluxo e manter a latência dentro dos limites aceitáveis. Em cenários reais há situações que levam a perda de pacotes, exemplos desses sendo: alta latência e \en{peer churn} \cite{Nasrallah2018ULL}; assim, na construção dos testes foi implementada aleatoridade do contexto da rede para simular cenários reais, juntamente com o agrupamento de \en{nodes} em subredes.

\subsection{ACERCA DO DESENVOLVIMENTO DA APLICAÇÃO}

A aplicação de teste foi desenvolvida usando \en{GoLang} para os \en{middlewares} de sincronização, \en{Kotlin} com a plataforma \en{Android Studio} para a aplicação móvel; para editor de código genérico foi usado o \en{Neovim}. O ambiente de testes consiste no \en{Docker} com configurações para auxiliar na simulação de \en{peer churn}; o sistema operacional \en{host} é um Debian 12. Para o banco de dados local da aplicação móvel foi usado o \en{Room} responsável por adicionar uma camada de abstração sobre o \en{SQLite}.

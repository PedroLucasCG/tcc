...
Esta seção apresenta conceitos necessários para o desenvolvimento do trabalho, sendo esses: descrição das tecnologias usadas para a construção da aplicação (\en{Go}, \en{Kotlin}, redes \en{Peer-to-Peer}), diferentes tipos de estratégias de sincronização e tópicos essenciais para o seu entendimento tão como comparações com trabalhos relacionados.
...
\en{Offline-first} é um paradigma de programação que define o comportamento de aplicações em relação a falta de conectividade \cite{OfflineFirst}. Um bom exemplo desse pradigma são os \en{Progressive Web Apps (PWA)}, para uma aplicação ser um \en{PWA} esta deve englobar conceitos que formulam o modo que foi construída; dentre eles destaca-se, para esse trabalho, a independência de conexão \cite{BjornHansen2018PWAUnifiedMobile}. A arquitetura define, também, o armazenamento local de dados como componente essencial para sua implementação, e com ele sincronização e a resolução de conflitos. \en{Offline-first} assume a existência da necessidade de conexão com alguma rede, sendo essa efêmera, e especifica como aplicações devem funcionar nesse contexto \cite{Guda2025OfflineFirstAndroidChat}.  
